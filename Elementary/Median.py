...
# of the two numbers found in the middle. For this mission, you are given a non-empty
# array of natural numbers (X). With it, you must separate the upper half of the numbers
...
def checkio(data: List[int]) -> [int, float]:
    data = sorted(data)
    if len(data) % 2 == 1:
        border1 = int(len(data) / 2)
        return data[border1]
    else:
        border2 = int((len(data)) / 2)
        med = (data[border2-1] + data[border2]) / 2
        return round(med, 1)
