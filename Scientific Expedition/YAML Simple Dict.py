...
# The first step is the key-value conversion. The key can be any string consisting of Latin letters
# and numbers. The value can be a single-line string (which consists of spaces, Latin letters and
# numbers) or a number (int).
...
# Pay attention to a few things. Between the string "age" and the string "class" there is an empty
# string that doesn’t interfere with parsing. The class starts with numbers, but has letters, which
# means it cannot be converted to numbers, so its type remains a string (str).
